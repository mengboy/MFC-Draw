...
应用程序向导已为您创建了此 Draw 应用程序。此应用程序不仅演示 Microsoft 基础类的基本使用方法，还可作为您编写应用程序的起点。
...
Draw.h
    这是应用程序的主头文件。
    其中包括其他项目特定的标头（包括 Resource.h），并声明 CDrawApp 应用程序类。

Draw.cpp
    这是包含应用程序类 CDrawApp 的主应用程序源文件。

Draw.rc
    这是程序使用的所有 Microsoft Windows 资源的列表。它包括 RES 子目录中存储的图标、位图和光标。此文件可以直接在 Microsoft Visual C++ 中进行编辑。项目资源包含在 2052 中。

res\Draw.ico
    这是用作应用程序图标的图标文件。此图标包括在主资源文件 Draw.rc 中。
...
DrawDoc.h、DrawDoc.cpp - 文档
    这些文件包含 CDrawDoc 类。编辑这些文件以添加特殊文档数据并实现文件保存和加载（通过 CDrawDoc::Serialize）。

DrawView.h、DrawView.cpp - 文档视图
    这些文件包含 CDrawView 类。
